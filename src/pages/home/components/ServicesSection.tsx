...
const SERVICES_DATA: Service[] = [
  {
    ...
    description: 'Our sourcing team works directly with factories and top-tier marketplaces to secure quality products at unbeatable prices.',
    features: [
...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, index }) => {
  ...
  return (
    <div
      className="bg-gradient-to-br from-gray-50 to-white p-8 rounded-2xl border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer flex flex-col items-center text-center group"
      ...
      <ul className="space-y-2 text-left w-full max-w-xs mx-auto">
        {service.features.map((feature, idx) => (
          <li key={idx} className="flex items-center text-sm text-gray-700">
            <i className="ri-check-line text-green-500 mr-2 w-5 h-5 flex items-center justify-center flex-shrink-0"></i>
            <span>{feature}</span>
          </li>
        ))}
      </ul>
